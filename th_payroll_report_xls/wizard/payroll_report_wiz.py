...
class payrollreportexcelwiz(models.TransientModel):
    ...
    from_date = fields.Date('Date debut', required=True)
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.merge_range('A1:F2', 'Paie du mois de %s %s' % (payroll_month, self.from_date.year), heading_format)
            worksheet.merge_range('B4:D4', '%s' % (self.company.name), cell_text_format_n)
            row = 2
            column = 0
            worksheet.write(row + 1, 0, 'Société', cell_text_format_n)
            worksheet.write(row, 4, 'Date debut', cell_text_format_n)
            worksheet.write(row, 5, date_1 or '')
            row += 1
            worksheet.write(row, 4, 'Date fin', cell_text_format_n)
            worksheet.write(row, 5, date_2 or '')
            row += 2
            res=self.get_rules()

            worksheet.write(row, 0, 'Employé', cell_text_format)
            worksheet.write(row, 1, 'Identifiant', cell_text_format)
...
class payroll_report_excel(models.TransientModel):
    ...
    file_download = fields.Binary('Telecharger la paie', readonly=True)
